    ...
    <Container>
      <div class="grid w-full grid-cols-1 gap-6 mb-10 md:flex md:justify-between md:grid-cols-3">
        <div class=" col-span-full lg:col-span-1 lg:block">
          <div>
            <router-link
              to="/" class="flex-none text-xl font-semibold text-white font-Orbit"
              aria-label="Brand"
            >
              Quickbyte
            </router-link>
          </div>
          <p class="mt-3 text-xs text-[#A1A1A1] sm:text-sm dark:text-gray-400">
        ...
        <div class="flex space-x-20">
          <div v-for="footer in footerNav" :key="footer.id">
            <h4 class="text-xs font-semibold text-white uppercase dark:text-gray-100">
              {{ footer.name }}
            </h4>
            <div v-for="nav in footer.children" :key="nav.id">
              <div class="grid mt-3 space-y-3 text-sm">
                <a
                  :href="nav.path" target="_blank"
